...
import { Plus, Activity, Settings, Edit2, Trash2, PieChart as PieChartIcon, EyeOff, Link, ChevronDown, ChevronUp, BarChart2, TrendingDown, Loader2 } from 'lucide-react';
import { AddBetModal } from './AddBetModal';
import { EditBankrollModal } from './EditBankrollModal';
import confetti from 'canvas-confetti';

// Lazy Load Recharts components to reduce initial bundle size and main thread blocking
...
// --- NEW COMPONENT: CHARTS SECTION (Memoized) ---
...
const ChartsSection = memo(({ bets, initialBankroll, analytics, isZenMode }: ChartsSectionProps) => {
            ...
            <div className="md:col-span-2 bg-slate-900/50 p-4 rounded-xl border border-slate-800 shadow-inner h-[300px]">
                ...
                <ResponsiveContainer width="100%" height="85%">
                    ...
                    <YAxis 
                        ...
                        tickLine={false}
                        width={35}
                        tickFormatter={(value: any) => isZenMode ? '***' : value}
                    />
                    <Tooltip 
...
export const StatsView: React.FC = () => {
  ...
  return (
    <div className="p-4 pb-24 space-y-6 animate-slide-up">
      ...
      {/* Main Charts Grid - Suspended to prevent blocking */}
      <Suspense fallback={
          <div className="h-[300px] w-full bg-slate-900/50 rounded-xl border border-slate-800 flex flex-col items-center justify-center animate-pulse gap-3">
             <Loader2 size={32} className="text-emerald-500 animate-spin" />
             <p className="text-xs text-slate-500 font-mono">Cargando visualización de datos...</p>
          </div>
      }>
        <ChartsSection 
            bets={bets} 
         ...
         <div className="space-y-3">
             {bets.length === 0 ? (
                 <div className="text-center py-8 text-slate-600 border border-dashed border-slate-800 rounded-xl bg-slate-900/30">
                     {/* Using a Lucide icon that is not lazy loaded, from the main import */}
